...
load_dir = "../2024_09_25/Experiment/MgO_sig_with_ref/i_Values" # Specify the location of the experimetal data
work_dir = "../work_2024_09_25"
...
Threads.@threads for i in eachindex(tau_range)
    ...
    for k in eachindex(T_range)
        filename = "$(load_dir)/T_$(motor_T[k])_tau_$(motor_tau[i])_scan_$(string(scan_range)).txt"
        data_array[:, k] = readdlm(filename)./10000 #To normalize the values (Can be number of pixels as well)
    end
    x_range, phi_range, pr_data = dataload2(data_array, (-4, 2), 1000) #Calculate the PDF from data
