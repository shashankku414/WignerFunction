...
num_frames = 100
num_frames_scans = num_frames*length(scan_range) #This is the number of frames times number of scans that is measured for tau

println("Number of available threads = $(Threads.nthreads())") # For parallel computing > 1
for i in eachindex(tau_range)
    data_array = zeros((num_frames_scans, length(T_range)))
    save_dir = "$(work_dir)/tau_$(tau_range[i])as"
    mkpath("$(save_dir)")
    for k in eachindex(T_range)
        Threads.@threads for j in eachindex(scan_range)
            ...
            data_array[(j-1)*num_frames+1:j*num_frames, k] = readdlm(filename)./10000
        end
    end
